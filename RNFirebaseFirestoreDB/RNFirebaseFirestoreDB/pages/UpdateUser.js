...
const UpdateUser = (props) => {
      ...
      firestore()
        ...
        .then((documentSnapshot) => {
          /*
            ...
            The snapshot provides the ability to view a documents data,
  ...
  const updateUser = () => {
    if (userId && userName && userContact && userAddress) {
      /*
        Please note update is not just for the update in firebase, while updating if record not found in firebase then
        it will create one, update Method also provides support for
        updating deeply nested values via dot-notation .update({ 'details.address.zipcode': 452012 })
      */

      firestore()
        .collection('Users')
        .doc(userId)
        .update({name: userName, contact: userContact, address: userAddress})
        .then(() => {
          Alert.alert(
            'Success',
  ...
  return (
    <View style={{backgroundColor: 'white', flex: 1, paddingHorizontal: 35}}>
